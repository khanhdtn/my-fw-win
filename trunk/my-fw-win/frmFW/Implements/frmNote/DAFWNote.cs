    ...
    public class DAFWNote :DAPhieu<DOFWNote>
    {
        ...
        public DOFWNote LoadFromUser(long UserID) {
            try
            {
                ...
                {
                    //Tạo cấu trúc trước 
                    if (HelpSQLScript.RunStringSQLScript(
                        @"CREATE TABLE FW_NOTE (
                            ID        A_BIG_ID NOT NULL /* A_BIG_ID = BIGINT */,
                            USERID    A_BIG_ID /* A_BIG_ID = BIGINT */,
                            NOI_DUNG  BLOB SUB_TYPE 0 SEGMENT SIZE 80 /* IMAGE = BLOB SUB_TYPE 0 SEGMENT SIZE 80 */
                        );
                        ALTER TABLE FW_NOTE ADD CONSTRAINT PK_FW_NOTE PRIMARY KEY (ID);
                    "))
                    {
                        //Tạo dữ liệu sau
                        HelpSQLScript.RunStringSQLScript(
                        @"INSERT INTO FW_NOTE (ID, USERID) VALUES (1, 1);
                        "
                        );
                    }
                    return LoadFromUser(UserID);
                }
                catch { }
            }
            return new DOFWNote();
